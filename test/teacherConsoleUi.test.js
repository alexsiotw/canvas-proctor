...
test('teacher asset versions invalidate cached pre-facelift files', () => {
  // Matched on the 4.7.x series rather than one exact patch number. The intent is
  // that these assets carry a cache buster past the pre-facelift build — pinning
  // 4.7.1 exactly meant any later bump failed this test, which is what happened
  // when the review-layout CSS fix required busting the stylesheet again.
  assert.match(indexSource, /styles\.css\?v=4\.7\.\d+/);
  assert.match(indexSource, /app\.js\?v=4\.7\.\d+/);
});
